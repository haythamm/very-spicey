...
def login():    #Logs user into space-track, using either default or temporary account.
    ...
    if choice == 'n':   #if used doesnt want to use default...
        ...
        print("Authenticating...")
        st_user = 'default_username_here'  # enter default spacetrack username here
        st_pass = 'default_password_here'  # enter default spacetrack password here
...
def setupFile(fileName):    #Generates a setup file for mkspk
    ...
    if choice == 'y':
        ...
        f.write("\tLEAPSECONDS_FILE\t= '/kernels/naif0011.tls'\n")
        f.write("\tINPUT_DATA_FILE\t\t= /" + fileName + "\n")
        f.write("\tOUTPUT_SPK_FILE\t\t= /" + fileName[:-4]+".bsp\n")
        f.write("\tPCK_FILE\t\t\t= '/kernels/geophysical.ker'\n")
